...
import numpy as np
from transformers import BertTokenizer  # New import
...
def collate_fn_transformer(batch, vocab):
    ...
    # Use the special pad token defined in our vocab (set in build_vocab)
...
class Encoder_Only_Transformer(nn.Module):
    def __init__(self, vocab_size, embedding_dim, n_heads=10, hidden_size=64, dropout=0.1, num_classes=2, num_blocks=3):
        super().__init__()
        # Use padding_idx=0 because we set our pad token id to 0 in the vocabulary
...
class TransformerPipeline:
    def __init__(self):
        ...
        # Initialize the pretrained BERT tokenizer for multilingual text
    ...
    def preprocess(self):
        """Tokenize text data using the pretrained BERT tokenizer"""
    ...
    def build_vocab(self):
        """Build vocabulary from the BERT tokenizer's vocabulary"""
        # Get the original BERT vocabulary (a dict mapping token to id)
        self.vocab = self.tokenizer.get_vocab()
        # For consistency with the rest of our code, add keys for pad and unknown tokens.
        self.vocab['<PAD>'] = self.tokenizer.pad_token_id
        self.vocab['<UNK>'] = self.tokenizer.unk_token_id
    ...
    def predictions(self):
        ...
        # Updated prediction function that uses the pretrained BERT tokenizer
        def predict_sentiment(model, sentence, tokenizer, vocab, max_length, device):
            model.eval()
            if not sentence.strip():
                return -1
            # Tokenize and convert to token ids using the BERT tokenizer
            tokens = tokenizer.tokenize(sentence)
            token_ids = tokenizer.convert_tokens_to_ids(tokens)
            
            # Padding/truncating using our special pad token id
            ...
            for idx, row in test_data.iterrows():
                ...
                try:
                    prediction = predict_sentiment(
                        self.test_model, 
                        sentence, 
                        self.tokenizer,  # Pass the pretrained tokenizer
                        self.vocab, 
                        max_length,
                        self.device
                    )
